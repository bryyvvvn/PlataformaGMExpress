'use client'; // Si en algún momento migran a Next.js App Router directo
...
const HomePage: React.FC = () => {
  ...
  // ── Calendario ────────────────────────────────────────────────────────────
  const {
    setSemanaOffset,      // Ahora gestiona cambio de semana + reset de día
    fechaTexto,
    diasSemanaArray,
    getSemanaTexto,
    diaSeleccionadoIdx,
    setDiaSeleccionadoIdx,
    fechaSeleccionadaISO, // 'YYYY-MM-DD' del día activo → va a la API
  } = useCalendario();
  ...
  // 🔑 Clave: se pasa fechaSeleccionadaISO → cuando el usuario toca otro día,
  //    useMenuAPI re-ejecuta el fetch automáticamente (ver useEffect con [fecha])
  ...
  // Resetear selecciones de platos cuando el usuario cambia de día
  useEffect(() => {
    setPedido({ entradaId: null, fondoId: null, postreId: null, guarnicionId: null });
    setSeccionAbierta(null);
  }, [fechaSeleccionadaISO]);

  // Controlar apertura automática de secciones
  useEffect(() => {
  ...
  const seleccionarPlato = (
    ...
    // For FONDO: if the fondo has guarniciones, open bottom sheet to choose.
    if (categoria === 'fondoId') {
      const seleccionado = (menuHoy.fondos || []).find((p: any) => p.id === id);
      if (seleccionado && seleccionado.guarniciones && seleccionado.guarniciones.length > 0) {
        // Open sheet and let user choose guarnicion; selection confirmed there
    ...
    setPedido(prev => {
      const newState = { ...prev, [categoria]: prev[categoria] === id ? null : id } as any;
      if (categoria !== 'fondoId') newState.guarnicionId = prev.guarnicionId;
      if (categoria === 'fondoId' && prev.fondoId !== id) newState.guarnicionId = null;
      return newState;
    });
  ...
  // Handler para seleccionar un día del calendario
  const seleccionarDia = (index: number) => {
    if (diaSeleccionadoIdx === index) return; // ya está seleccionado
    setDiaSeleccionadoIdx(index);
    // El useEffect de fechaSeleccionadaISO limpia el pedido y cierra secciones
  };
  ...
  const estaCompleto = Boolean(
    pedido.entradaId && pedido.fondoId && pedido.postreId && (!fondoNeedsGuarnicion || pedido.guarnicionId !== null)
  );
  ...
  return (
    ...
    >
      ...
      <div className="mt-10 px-6" onClick={e => e.stopPropagation()}>
        ...
        {/* CALENDARIO DE DÍAS — ahora cada día es tappable */}
      ...
      {/* LISTADO DE PLATOS */}
      <div
        className={`mt-10 px-6 space-y-4 ${
          yaPedioHoy || isDeadlinePassed ? 'opacity-40 grayscale pointer-events-none' : ''
        }`}
      >
        {/* Indicador de carga del menú del día seleccionado */}
        {cargandoMenu ? (
      ...
      {/* Bottom sheet para selección de guarnición (cuando se abre) */}
      {sheetFondo && (
        <BottomSheet open={sheetOpen} title={`Guarniciones — ${sheetFondo.nombre}`} onClose={() => setSheetOpen(false)}>
          <div className="px-2">
            <div className="flex gap-2 flex-wrap">
              {(sheetFondo.guarniciones || []).map((g: any) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-xl border text-sm font-bold ${sheetSelectedGuarnicion === g.id ? 'bg-green-600 text-white' : 'bg-white text-gray-700'}`}
                >
                  {g.nombre}
                </button>
              ))}
              <button
                ...
                className={`px-4 py-3 rounded-xl border text-sm font-bold ${sheetSelectedGuarnicion === -1 ? 'bg-green-600 text-white' : 'bg-white text-gray-700'}`}
              >
                Sin guarnición
              </button>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                onClick={() => {
                  // Confirm selection: set pedido.fondoId and guarnicionId
                  setPedido(prev => ({ ...prev, fondoId: sheetFondo.id, guarnicionId: sheetSelectedGuarnicion }))
                  setSheetOpen(false);
                  setSeccionAbierta('POSTRE');
                }}
                disabled={sheetSelectedGuarnicion === null}
                className={`px-4 py-3 rounded-xl font-black ${sheetSelectedGuarnicion === null ? 'bg-gray-100 text-gray-400' : 'bg-green-600 text-white'}`}
              >Seleccionar</button>
            </div>
          </div>
        </BottomSheet>
